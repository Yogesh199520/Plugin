...
;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype = {
		...
		init: function () {
			...
			this.$smpnav.removeClass( 'smpnav-nojs' );		//We're off and running

			//this.$toggles = $( '.smpnav-toggle[data-smpnav-target="'+this.$smpnav.attr('id')+'"]' );
      ...
      this.initializeSubmenuToggleKeyboardEvents();
		...
		initializesmpnav: function(){
			...
			if( !$body.hasClass( 'smpnav-enabled' ) ){
				...
				if( smpnav_data.shift_body != 'off' ){
					if( smpnav_data.shift_body_wrapper != '' ){
						$( smpnav_data.shift_body_wrapper ).addClass( 'smpnav-wrap' );
					}
					else{
				...
				else $body.addClass( 'smpnav-disable-shift-body' );
        ...
        if( $main_toggle.length ){ //Only mess with gap if there is a main toggle bar in use
  				if( ( !$main_toggle.hasClass( 'smpnav-toggle-style-burger_only') && $main_toggle.hasClass( 'smpnav-togglebar-gap-auto' ) ) ||
  					$main_toggle.hasClass( 'smpnav-togglebar-gap-on' ) ){
            ...
            if( smpnav_data.shift_body == 'off' ){
                ...
                var style = 'body.smpnav-disable-shift-body{ padding-top:'+ toggleHeight + 'px; }';
      ...
      // this.$smpnav.on( 'focusin' , function(){
      //   if( !$(this).hasClass( 'smpnav-open-target' ) ){
      //     plugin.opensmpnav();
      //   }
      // });
      ...
      //Current open
		...
		initializeTargets: function(){
			...
			this.$smpnav.find( '.smpnav-scrollto' )
      ...
      // Retractors are also targets, so only listen on menu items
			this.$smpnav.on( 'click' , '.menu-item > .smpnav-target' , function( e ){
				...
				if( scrolltarget ){
					...
					if( $target.length > 0 ){
						...
						function(){
							plugin.closesmpnav();	//close the menu after a successful scroll
						});
		...
		initializeSubmenuToggleMouseEvents: function(){
			...
			this.$smpnav.on( 'mouseup.shift-submenu-toggle' , '.smpnav-submenu-activation' , function(e){ plugin.handleMouseActivation( e , this , plugin ); } );
			//$smpnav.on( 'mouseout.shift-submenu-toggle'  , '.menu-item' , this.handleMouseout );	//now only added on mouseover
		},

		disableSubmenuToggleMouseEvents: function(){
			if( this.settings.debug ) console.log( 'disableSubmenuToggleMouseEvents' );
			$smpnav.off( 'mouseover.shift-submenu-toggle' );
			$smpnav.off( 'mouseout.shift-submenu-toggle'  );
		},
    ...
    initializeSubmenuToggleKeyboardEvents: function(){
			...
			this.$smpnav.on( 'keyup.shift-submenu-toggle' , '.smpnav-submenu-activation' , function(e){
        if( e.keyCode === 13 ){ //return/enter
		...
		initializeRetractors: function() {
			...
			this.$smpnav.on( 'mouseup.smpnav' , '.smpnav-retract' , function(e){ plugin.handleSubmenuRetractorEnd( e , this, plugin); } );
      ...
      if( 'smpnav-main' === this.$smpnav.attr('id') ){
		...
		toggle: function( $toggle , plugin , e ){
			...
			if( plugin.$smpnav.hasClass( 'smpnav-open-target' ) ){
				...
				if( ( ( toggle_id == 'smpnav-toggle-main-button' ) ||
					  ( toggle_id == 'smpnav-toggle-main' ) ) &&
					$( 'body' ).hasClass( 'smpnav-open' ) ){
						//Close all smpnavs
		...
		opensmpnav: function( tag ){
			...
			$( 'body' )
				.removeClass( 'smpnav-open-right smpnav-open-left' )
				.addClass( this.openclass )
				.addClass( 'smpnav-transitioning' );

			//console.log( 'close ' + $( '.smpnav-open-target' ).attr( 'id' ) );
			$( '.smpnav-open-target' ).removeClass( 'smpnav-open-target' );
			this.$smpnav
				.addClass( 'smpnav-open-target' )
      ...
      $(document).on( 'keyup.smpnavPanel', function(e){
		...
		closesmpnav: function(){
			...
			this.$smpnav
				.removeClass( 'smpnav-open-target' )
				.on( plugin.transitionend, function(){
						//if( plugin.settings.debug ) console.log( 'finished submenu close transition' );
						$( 'body' ).removeClass( 'smpnav-transitioning' );
						$( this ).off( plugin.transitionend );
    ...
    focusMainToggle: function(plugin){
      if( this.$smpnav.attr('id') === 'smpnav-main' ){
		...
		initializeTouchoffClose: function(){
			...
			var plugin = this;
			$( document ).on( 'click.smpnav ' + this.touchEnd + '.smpnav' , function( e ){ plugin.handleTouchoffClose( e , this , plugin ); } );
		...
		openSubmenu: function( $li , tag , plugin ){
			if( !$li.hasClass( 'smpnav-active' ) ){
				...
				if( $li.hasClass( 'smpnav-sub-shift' ) ){
					...
					plugin.$smpnav.addClass( 'smpnav-sub-shift-active' );
				...
				$li.find( '> ul' )
					.addClass( 'smpnav-sub-active' )
					.addClass( 'smpnav-sub-active-current' );
		...
		closeSubmenu: function( $li , tag , plugin ){
			...
			if( $li.hasClass( 'menu-item-has-children' ) && $li.hasClass( 'smpnav-active' ) ){
				...
				$li.addClass( 'smpnav-in-transition' );	//transition class keeps visual flag until transition completes
			...
			//Shift Sub Specific
			if( $li.hasClass( 'smpnav-sub-shift' ) ){
				if( $li.parents( '.smpnav-sub-shift' ).length == 0 ) plugin.$smpnav.removeClass( 'smpnav-sub-shift-active' );
			...
			//Active flags
			$li.find( '> ul' )
				.removeClass( 'smpnav-sub-active' )
				.removeClass( 'smpnav-sub-active-current' );
...
(function($){
	...
	function initialize_smpnav( init_point ){
      ...
      //var $li = $( '.smpnav .smpnav-target[data-smpnav-scrolltarget="'+hash+'"]' ).parent();
      //Do it this way to avoid passing user-editable content into the $() function - find() is safer
      var $li = $( '.smpnav' ).find( '.smpnav-target[data-smpnav-scrolltarget="'+hash+'"]' ).parent();
			if( $li.length ){
				//console.log( $li );
				$li.siblings().removeClass( 'current-menu-item' ).removeClass( 'current-menu-ancestor' );
				$li.addClass( 'current-menu-item' );
