...
function smpnav_togglebar($toggle_target = 'smpnav-main', $content = '', $args = array())
{
    extract(shortcode_atts(array(
        ...
        //'menu_type' => smpnav_op('menu_type', 'general'),
    ), $args))
...
<?php
    ...
    if(!empty($hamburger_size)) {
        echo '<style>#smpnav-toggle-main #smpnav-toggle-main-button i {font-size: '.$hamburger_size.'px!important;} </style>';
    }
    if(!empty($font_size)){
        echo '<style>.smpnav-item a.smpnav-target, a.nav-link{font-size:'.$font_size.'px!important;}</style>';
    } 
    if(!empty($font_family)){
        echo '<style>.smpnav-item a.smpnav-target, .smpnav-item a.nav-link{font-family:"'.$font_family.'"!important;}</style>';
    }
    if(!empty($background_color)){
       echo '<style>#smpnav-toggle-main{background-color: '.$background_color.'!important;} </style>';
    }
    if(!empty($text_color)){
       echo '<style>ul.smpnav-menu li.menu-item > .smpnav-target,a.nav-link{color:'.$text_color.'!important;}</style>';
       echo '<style>#smpnav-toggle-main #smpnav-toggle-main-button i{color:'.$text_color.'!important;}</style>';
    }
     if(!empty($menu_item_bg)) {
        echo '<style>.smpnav.smpnav-skin-standard-dark, .smpnav.smpnav-skin-standard-dark ul.smpnav-menu {background-color: '.$menu_item_bg.'!important;} </style>';
    }
...
function smpnav_load_include()
{
    ...
    if (SCRIPT_DEBUG)
    {
        ...
        wp_enqueue_style('bootstrap4', 'https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css');
    ...
    {
        wp_enqueue_style('smpnav-font-awesome', $include . 'https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css', false, SMPNAV_VERSION);
    }
    ...
    if (SCRIPT_DEBUG)
    {
        wp_enqueue_script( 'popper-min4','https://cdn.jsdelivr.net/npm/popper.js@1.16.1/dist/umd/popper.min.js', array( 'jquery' ),'',true );
        wp_enqueue_script( 'bootstrap-js4','https://cdn.jsdelivr.net/npm/bootstrap@4.5.3/dist/js/bootstrap.min.js', array( 'jquery' ),'',true );
        wp_enqueue_script( 'popper-min5','https://cdn.jsdelivr.net/npm/@popperjs/core@2.9.2/dist/umd/popper.min.js', array( 'jquery' ),'',true );
        wp_enqueue_script( 'bootstrap-js5','https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.min.js', array( 'jquery' ),'',true );
...
function smpnav_get_menu_args($args, $id = 0)
{
    ...
    //self::smpnav_togglebar();
    //echo $this->menu_type;
    $args['container_class'] = 'smpnav-nav';
    $args['container'] = 'nav';
    $args['menu_class'] = 'smpnav-menu';
    $args['menu_type'] = smpnav_op('menu_type', 'general');
    $type = $args['menu_type'];
    if($type =='bs5'){
        $args['walker'] = new BS5NavWalker;
    }elseif($type =='bs4'){
