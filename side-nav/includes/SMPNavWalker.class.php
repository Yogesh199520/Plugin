...
class SMPNavWalker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$class_names = $class_names ? ' class="smpnav-item ' . esc_attr( $class_names ) . '"' : '';
		...
		if($this->has_children && $depth ==0) {
			$anchor_class = 'smpnav-target dropdown-menu';
		}elseif($this->has_children && $depth ==1) {
			$anchor_class = 'smpnav-target dropdown-dropdown-menu';
		}elseif($this->has_children && $depth ==2) {
			$anchor_class = 'smpnav-target dropdown-menu-level2';
		}elseif($this->has_children && $depth ==3) {
			$anchor_class = 'smpnav-target dropdown-menu-level3';
		}else{
		...
		if($this->has_children && $depth ==0) {
			$item_output .='<i class="icon-placement-level-zero fa fa-plus"></i>';
		}elseif($this->has_children && $depth ==1){
			$item_output .='<i class="icon-placement-level-one fa fa-plus"></i>';
		}elseif($this->has_children && $depth ==2){
			$item_output .='<i class="icon-placement-level-two fa fa-plus"></i>';
		}
